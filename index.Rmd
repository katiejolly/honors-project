...
Inhomogeneity of the partion with respect to the regionalization can be written out generally as: $$\sum_{y=1}^m\bigg(\frac{A_y}{A}\bigg)\frac{\text{variance of regions in zone y}}{\text{variance of regions in the domain}}$$
...
$$h = 1 - \sum_{y=1}^m\bigg(\frac{A_y}{A}\bigg)\frac{S^R_y}{S^R}$$
...
$$c = 1 - \sum_{z=1}^n\bigg(\frac{A_z}{A}\bigg)\frac{S^W_z}{S^W}$$
...
Below is an example taken from [Spatial association between regionalizations using theinformation-theoreticalV-measure by Nowosad and Stepinski](https://eartharxiv.org/rcjh7/) to illustrate how v-measure is calculated in practice. 

<br>
<br>

<img width="75%" src="C:\\Users\\katie\\Documents\\honors\\paper\\images\\vmeasure.PNG"/>
...
We start by initially just running the algorithm once to classify all of the blocks with enough neighbors at the start.

```{r eval = FALSE, include = TRUE, echo = TRUE}
# load the voterfile data (csv)

county_voterfile <- load_voterfile(county = "Noble")

# load the census block shapefile for a particular county

county_blocks <- load_blocks(county = "Noble")

# geocode the voterfile addresses with a census api to get the census block ID for each address
...
This group is certainly not the only one working on voting rights and data science for social good. Here is a starting list of other organizations and resources to know about if you're interested in this work! I've also included some articles that were particularly informative for me. 
...
* [An open letter to the open data community by the Civic Analytics Network](https://datasmart.ash.harvard.edu/news/article/an-open-letter-to-the-open-data-community-988)

* [Categorical spatial interpolation with R by Timo Grossenbacher](https://timogrossenbacher.ch/2018/03/categorical-spatial-interpolation-with-r/)

* [Understanding Census Bureau address ranges](https://www.census.gov/newsroom/blogs/research-matters/2016/04/understanding-census-bureau-address-ranges.html)

* [Drawing the line: How redistricting turned America from blue to red](https://www.newyorker.com/magazine/2016/06/27/ratfcked-the-influence-of-redistricting)

* [Automated georeferencing of historic aerial photography by J.S. Kim, C. C. Miller, and J. Bethel](https://docs.lib.purdue.edu/cgi/viewcontent.cgi?article=1066&context=jto)
...
You can adapt this code to any other Ohio county by starting with a different [voterfile link](https://www6.sos.state.oh.us/ords/f?p=111:1). 
...
vec <- purrr::map_chr(1:nrow(noble), function(i) tigris::call_geolocator(
    noble[["RESIDENTIAL_ADDRESS1"]][i],
...
classify_nn <- function(x, alpha = 0) {
  ...
  precincts_nb_sub <- x %>%
    filter(
      ...
      less_than_alpha == FALSE
    ) # take out blocks with not enough classified neighbors
  ...
  precincts_nb_sub <- precincts_nb_sub %>%
    mutate(
      precinct_nn_clean = p_new,
      precinct_nn_length = p_lengths
    ) %>% # finds the number of classified neighbors
