...
This ensures that spurious patterns from classifying with little data can be mitigated. Adding a minimum value that is too high prevents the algorithm from running to completion in some cases, so finding a balance is often a process of trial and error. We recommend a minimum percent ($\alpha$) no higher than 33 percent. 
...
One question that we had was how robust the algorithm was to changes to the minimum required neighbors. We tested $\alpha \in \{0, 0.25, 0.33, 0.4, 0.5, 0.8\}$. 

In general we found that $\alpha > 0.33$ was too strict. There would be a point at which there were no unclassified blocks that met the requirements and the percent classified would reach a horizontal asymptote (see figure below). 
