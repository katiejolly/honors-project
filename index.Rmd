...
In this section I will walk through how this algorithm works for one particular block. This example will focus on the center block of this area. Its neighbors are denoted by lines across the shared boundary. 

<img width="100%" align = "middle"  src="C:\\Users\\katie\\Documents\\honors\\paper\\images\\rook_example.PNG"/>
...
As with other machine learning methods, we consider the bias variance tradeoff in this model based on the parameter value $\alpha$. In this case, we do not consider $k(x_i)$ directly because the number of neighbors used for classification is not necessarily the size of a block's neighborhood. If $\alpha$ is low, and therefore we can use a small subset of neighbors, our estimates will be highly variable depending on what data we have available initially. A few differences in our initial data can change our estimates significantly since the process works iteratively.  
